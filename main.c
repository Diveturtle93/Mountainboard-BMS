...
//----------------------------------------------------------------------
volatile uint8_t millisekunden_flag_1 = 0;								// Flag um 1ms Task zu speichern und auszuf�hren
uint8_t rx0_receive = 0;									    		// Variable um UART0 Daten zu speichern
volatile uint8_t asyncronExtension = 0;									// Variable f�r TIMER2, Sleep-Mode
//----------------------------------------------------------------------
...
ISR(TIMER1_OVF_vect)
{
	millisekunden_flag_1 = 1;											// Nach ausl�sen des Interrupt setzen des Flags
...
//----------------------------------------------------------------------
ISR(USART_RX_vect)														// USART0 f�r ATmega128, USART f�r ATmega328
{
	rx0_receive = UDR0;													// Empfangene Daten in Variable speichern
}
//----------------------------------------------------------------------

// TIMER2 Overflow Interrupt-Routine f�r Sleep-Mode
//----------------------------------------------------------------------
/*ISR(TIMER2_OVF_vect)
{
...
int main(void)
{
	...
	// Variablen definieren
	uint8_t modus = 2, eeCounter = 0;									// Z�hler f�r Programm, 8 Bit
	uint16_t count = 0;													// Z�hler f�r Programm, 16 Bit
	uint16_t temperatur[2] = {0};										// Array um Temperatur zu speichern
	uint16_t spannungen[12] = {0};										// Array um Spannungen zu speichern
	uint8_t data[32] = {0};												// Array um Daten zu �bertragen
	uint16_t temp = 0;													// Tempor�re Variablen
	...
	{
		temp = ltc6804_check();											// LTC6804 Selbsttest durchf�hren
		if ((temp != 0) && (i == 2))									// Ausf�hren wenn Selbsttest dreimal fehlschl�gt
		{
			uart0_string("Selbstest fehlgeschlagen\r\n");				// Ausgabe bei Fehlerhaftem Selbsttest
			TCCR1B = 0;													// Timer Stoppen
			
			eeCounter = eeprom_read_byte(&eeFehlerZaehler);				// Z�hler f�r Fehlerspeicher auslesen
			eeprom_write_byte(&eeFehlerSpeicher[eeCounter], temp);		// Fehler speichern
			eeCounter--;
			eeprom_write_byte(&eeFehlerZaehler, eeCounter);				// Fehlerz�hler herunterz�hlen
	...
	// Alle Register zur�cksetzen
	ltc6804(CLRCELL);
	ltc6804(CLRSTAT);
	ltc6804(CLRAUX);
	
	ltc6804(ADCVAX | MD73 | CELLALL);									// Initial Command Zellen auslesen; Daten fallen lassen, da nicht ben�tigt
	...
	while(1)
	{
		...
		{
			modus = 1;													// Modus 1
			rx0_receive = 0;											// Receive Variable zur�cksetzen
		}
		
		// Task wird jede Millisekunde ausgef�hrt
		if (millisekunden_flag_1 == 1)
		{
			count++;													// Z�hle count hoch
			millisekunden_flag_1 = 0;									// Setze Millisekunden-Flag zur�ck
		}
		
		// Task wird alle 500ms ausgef�hrt
		if ((count % 500) == 0)
		{
		...
		// Task wird alle 2s durchgef�hrt			(Zeit zum balancen muss kleiner 1,5s sein. Sonst bricht der IC ab)
		if ((count % 2000) == 0)
		{
			...
			for (uint8_t i = 0; i < 12; i++)
			{
				...
				{
					V_min = spannungen[i];								// Wenn Zellsapnnug kleiner V_min ist, V_min �berschreiben
					min = i;											// Minimalzelle zwischenspeichern
				}
				
				if (spannungen[i] > V_max)								// Zellspannugen mit V_max vergleichen
				{
					V_max = spannungen[i];								// Wenn Zellsapnnug gr��er V_max ist, V_max �berschreiben
		...
		// Task wird alle 2s durchgef�hrt, unter der Bedingung das Serielle Ausgabe gew�nscht ist
		if (((count % 2000) == 0) && (modus == 1))
		{
			for (uint8_t i = 0; i < 14; i++)							// Z�hler hochz�hlen um Arraywert auszuw�hlen
			{
				if (i >= 12)
					temp = temperatur[i-12];							// Wenn Z�hlerwert gr��er 12 ist Temperaturwert ausw�hlen
				else
					temp = spannungen[i];								// Solange Z�hlerwert kleiner 12 ist Zellspannung ausw�hlen
		...
		// Task wird alle 10s durchgef�hrt
		if(count == 10000)
		{
			count = 0;													// Timer Counter zur�cksetzen
		}
		// Ende 10s
